...
{	
	...
	public static void main(String[] args) throws URISyntaxException {
		...
		try {
			PhotonEngine.loadClient("151.80.57.82");
			Thread.sleep(1000);
		...
		final String ending = FilenameUtils.getExtension(currentExecutionFile.getPath());
    	if(PhotonInfosManager.hasLauncherUpdate(LauncherEngine.VERSION)) {
    		...
    		while(!PhotonInfosManager.updateFinished) {
    			if(PhotonInfosManager.updateSize > 0 && PhotonInfosManager.updateSizeDownloaded >= PhotonInfosManager.updateSize) ApplicationUtils.launch(new File(currentExecutionFolder, "/launcher-" + PhotonInfosManager.getLatestLauncherUpdate() + "."+ending), new String[] { "delfile-"+currentExecutionFile.getPath() }, true);
